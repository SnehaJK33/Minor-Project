...
function normalizeFuturePrediction(future, history) {
  ...
  if (invalid) return { years: [], rates: [], used_fallback: false };
...
analyzeBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const dataJson = await fetchJSON(`${BASE_URL}/data/${encodeURIComponent(location)}?${cacheBust}`);
    const summaryJson = await fetchJSON(`${BASE_URL}/summary/${encodeURIComponent(location)}?${cacheBust}`);
    let gainJson = { gain_ha: 0 };
    try {
      gainJson = await fetchJSON(`${BASE_URL}/district/gain/${encodeURIComponent(location)}?${cacheBust}`);
    } catch (gainErr) {
    ...
    const predictionMeta = {
      ...
      cvR2: toNullableNumber(future.cv_r2),
      cvMae: toNullableNumber(future.cv_mae),
      cvRmse: toNullableNumber(future.cv_rmse),
      reliabilityStatus: String(future.reliability_status || "").trim(),
      backendFallback: !!future.used_fallback,
      trainingSamples: toNullableNumber(future.training_samples),
      predictionSource: summaryJson.prediction_source || ""
    };
    ...
    predictionMeta.frontendFallback = false;
    predictionMeta.usedFallback = !!predictionMeta.backendFallback;
...
function renderSummary(summary, environment, risk, location, history = [], predictionMeta = {}) {
  ...
  const showCv = (hasNumber(predictionMeta.cvR2) || hasNumber(predictionMeta.cvMae) || hasNumber(predictionMeta.cvRmse)) && predictionMeta.predictionSource !== "district_trend";
  const cvHtml = showCv
    ? `
    <p><b>Validation R� (CV):</b> ${hasNumber(predictionMeta.cvR2) ? predictionMeta.cvR2.toFixed(3) : "N/A"}</p>
    <p><b>Validation MAE (CV):</b> ${hasNumber(predictionMeta.cvMae) ? predictionMeta.cvMae.toFixed(3) : "N/A"}</p>
    <p><b>Validation RMSE (CV):</b> ${hasNumber(predictionMeta.cvRmse) ? predictionMeta.cvRmse.toFixed(3) : "N/A"}</p>
  `
    : "";
  const sourceLabel = predictionMeta.frontendFallback
    ? "Frontend fallback trend (used due to flat/invalid API forecast)"
    : (predictionMeta.predictionSource ? `Backend model output (${predictionMeta.predictionSource})` : "Backend model output");
  summaryDiv.innerHTML = `
    ...
    <p><b>Forecast Reliability:</b> ${predictionMeta.reliabilityStatus || "Unknown"}${predictionMeta.backendFallback ? " (backend fallback used)" : ""}</p>
    ${cvHtml}
    <p><b>Training R²:</b> ${hasNumber(predictionMeta.r2) ? predictionMeta.r2.toFixed(3) : "N/A"}</p>
    <p><b>Training Samples:</b> ${hasNumber(predictionMeta.trainingSamples) ? Math.round(predictionMeta.trainingSamples) : "N/A"}</p>
    <p><b>Prediction Source:</b> ${sourceLabel}</p>
...
function renderCharts(history, future, gainHa = 0) {
  ...
  // -------------------------------
  // 2?? Prediction Chart (Forecast Only)
  // -------------------------------
  const predYears = (future.years || []).map(v => toNumber(v)).filter(Number.isFinite);
  const predRates = (future.rates || []).map(v => toNumber(v)).filter(Number.isFinite);
  const predLower = (future.lower || []).map(v => toNumber(v)).filter(Number.isFinite);
  const predUpper = (future.upper || []).map(v => toNumber(v)).filter(Number.isFinite);
  const hasBand = predLower.length === predRates.length && predUpper.length === predRates.length;

  const predDatasets = hasBand ? [
    {
      label: "Upper band",
      data: predUpper,
